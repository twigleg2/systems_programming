...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        int s = getaddrinfo(host, port ? port : "80", &hints, &result); //host and port come from the original http GET request
        if (s != 0)
        {
    ...
    and) try the next address. */
        ...
        read(sfd3, content, MAX_OBJECT_SIZE); // TODO: robust read?
        printf("content: %s\n", content);
