...
Gaven Finch - gfinch
a) 4 cores
1 thread:  30.7 seconds
2 threads: 17.0 seconds
4 threads: 12.3 seconds
8 threads: 8.9 seconds
*/
...
int main(int argc, char *argv[])
{
  ...
  int **pic = malloc(yres * sizeof(int *));
  for (int r; r < yres; r++)
  {
    pic[r] = malloc(xres * sizeof(int));
  }
  double start = omp_get_wtime();
  omp_set_dynamic(0);
  omp_set_num_threads(8);
#pragma omp parallel for shared(pic) private(x, y, u, v, i, j, k) schedule(static)
  for (j = 0; j < yres; j++)
  {
    y = ymax - j * dy;
    for (i = 0; i < xres; i++)
